...
function createDisplayTable(where,inArray)
{
  ...
  tstring.push('<table style="padding: 2px; border:none" class="CTB"><tbody>');
...
function createDisplayTable2(where,inArray)
{
  ...
  tstring.push('<td class="c"><p class="CBwarn" style="font-size: 24px;">' + inArray[0] + ' <span style="color:green"> Reps &nabla; ' + inArray.length +'</span></p></td>');
  ...
  for (var i = 1; i <= this.alarmTimes[FN.NUMBERALARMS]; i++)
  {
    if( (this.alarmTimes[i][myField.IALARMTYPE]) == miAlarmType)
    {
      if((this.alarmTimes[i][myField.MALARMTIME]) != 0)
      {
        ...
        {
          doMessage('We have not worked out how to display more than 5 Timers, We are working on it, Send Cash now');
          return -1;
        }
      }
    }
...
function storeResults( AlarmNum , sName, c, c1)
{
  ...
  */
  try
  {
    ...
    var col1 = Trim('#Alarm ' + AlarmNum + " " + sName);
    ...
    col2 = Trim('Done at ' + col2);

    c1.appendChild(makerow(col1, col2 ));
    c.appendChild( c1 );
    var aa = document.getElementById('place');
...
function makecell(cellin)
{
  ...
  var c4 = document.createTextNode(cellin);
  c3.appendChild( c4 );
